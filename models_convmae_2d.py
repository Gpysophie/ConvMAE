...
from vision_transformer import PatchEmbed2d, Block, CBlock2d
...
class MaskedAutoencoderConvViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.stage1_output_decode = nn.Conv2d(embed_dim[0], embed_dim[2], (patch_size[1] * patch_size[2], 1), stride=(patch_size[1] * patch_size[2], 1))
        self.stage2_output_decode = nn.Conv2d(embed_dim[1], embed_dim[2], (patch_size[2], 1), stride=(patch_size[2], 1))
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim[2]), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_1d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.patch_embed3.num_patches), cls_token=True)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        decoder_pos_embed = get_1d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], int(self.patch_embed3.num_patches), cls_token=True)
...
#        torch.nn.init.normal_(self.cls_token, std=.02)
    ...
    def patchify(self, imgs):
        ...
        h = imgs.shape[2] // p
        x = imgs.reshape(shape=(imgs.shape[0], imgs.shape[1], h, p))
        x = torch.einsum('nchp->nhpc', x)
        x = x.reshape(shape=(imgs.shape[0], h, p * imgs.shape[1]))
        return x
...
#        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
    ...
    def forward_encoder(self, x, meta, mask_ratio):
        # embed patches
        ids_keep, mask, ids_restore = self.random_masking(x, mask_ratio)
        mask_len = mask.shape[-1]
        patch1_ratio = self.patch_size[1] * self.patch_size[2]
        patch2_ratio = self.patch_size[2]

        x = x.unsqueeze(-1)
        mask_for_patch1 = mask.unsqueeze(-1).repeat(1, 1, patch1_ratio).reshape(x.shape[0], patch1_ratio * mask_len, 1).unsqueeze(1)
        mask_for_patch2 = mask.unsqueeze(-1).repeat(1, 1, patch2_ratio).reshape(x.shape[0], patch2_ratio * mask_len, 1).unsqueeze(1)
        ...
        for blk in self.blocks3:
            x = blk(x)
        x_tmp = x[:, 1:, :] + stage1_embed + stage2_embed
        x = torch.cat((x[:, :1, :], x_tmp), dim=1)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # with cls token
    ...
    def forward(self, imgs, meta, mask_ratio=0.75, file_path=''):
        meta = torch.concat((meta[:, :4], meta[:, 5, None]), dim=1)
        latent, mask, ids_restore = self.forward_encoder(imgs, meta, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(imgs, pred, mask)
        return loss, pred, mask


def convmae_convvit_tiny_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderConvViT(
        embed_dim=[64, 128, 256], depth=[1, 1, 7], num_heads=8,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=8,
        mlp_ratio=[2, 2, 2], norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def convmae_convvit_small_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderConvViT(
        embed_dim=[64, 128, 256], depth=[2, 2, 8], num_heads=8,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=8,
        mlp_ratio=[2, 2, 2], norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def convmae_convvit_large_patch16_dec512d12b(**kwargs):
    model = MaskedAutoencoderConvViT(
        embed_dim=[384, 768, 1024], depth=[4, 4, 12], num_heads=16,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=[4, 4, 4], norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
convmae_convvit_tiny_patch16 = convmae_convvit_tiny_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
convmae_convvit_small_patch16 = convmae_convvit_small_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
convmae_convvit_base_patch16 = convmae_convvit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
convmae_convvit_large_patch16 = convmae_convvit_large_patch16_dec512d12b  # decoder: 512 dim, 8 blocks
